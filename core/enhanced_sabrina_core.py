...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("logs/sabrina_core.log"), logging.StreamHandler()],
)
...
class SabrinaCore:
    ...
    def __init__(self, config_path: str = "config/settings.yaml"):
        ...
        logger.info(
            f"Enhanced Sabrina AI Core initialized in {startup_duration:.2f} seconds"
        )
    ...
    def _init_memory(self):
        ...
        self.memory_compaction_threshold = memory_config.get(
        ...
        try:
            if os.path.exists(self.memory_file):
                with open(self.memory_file, "r") as f:
                    history = json.load(f)
                logger.info(
                    f"Loaded {len(history)} history entries from {self.memory_file}"
                )
                return history
            return []
        except Exception as e:
    ...
    def _save_history(self):
        ...
        try:
            ...
            logger.debug(
                f"Saved {len(self.history)} history entries to {self.memory_file}"
            )
        except Exception as e:
            self.error_handler.log_error(
                e,
    ...
    def update_memory(
        ...
        try:
            ...
            if (
                len(self.history) > self.max_history_entries * 2
            ):  # *2 because each exchange has 2 entries
            ...
            self._update_memory_index(
                user_input, len(self.history) - 2
            )  # Index points to user message
    ...
    def _init_components(self):
        ...
        enabled_components = core_config.get(
            "enabled_components", ["voice", "vision", "hearing", "automation"]
        )
        ...
        for component_name in enabled_components:
            try:
                ...
                if not all(dep in self.components for dep in dependencies):
                    missing_deps = [
                        dep for dep in dependencies if dep not in self.components
                    ]
                    logger.warning(
                        f"Cannot initialize {component_name} - missing dependencies: {missing_deps}"
                    )
                ...
                if component:
                    ...
                    logger.info(
                        f"{component_name.capitalize()} component initialized successfully"
                    )
                else:
        ...
        if critical_failures:
            logger.critical(
                f"Failed to initialize critical components: {critical_failures}"
            )
            self.critical_init_failure = True
        else:
            if failed_components:
                logger.warning(
                    f"Some non-critical components failed to initialize: {failed_components}"
                )
            self.critical_init_failure = False
    ...
    def _init_automation(self):
        ...
        try:
            ...
            if hasattr(actions, "configure"):
                actions.configure(
                    mouse_move_duration=automation_config.get(
    ...
    def _init_smart_home(self):
        ...
        try:
            ...
            smart_home = smart_home_class(
                home_assistant_url=home_assistant_url, event_bus=self.event_bus
            )
            ...
            if hasattr(smart_home, "test_connection"):
                ...
                if connected:
                    ...
                    logger.warning(
                        f"Failed to connect to Home Assistant at {home_assistant_url}"
                    )
    ...
    def _voice_to_presence_handler(self, event):
        ...
        """
        if "presence" not in self.components or not hasattr(
            self.components["presence"], "set_animation"
        ):
            return
    ...
    def _change_state(self, new_state):
        ...
        # Record state change
        state_change = {"from": old_state, "to": new_state, "timestamp": time.time()}
    ...
    def _register_event_handlers(self):
        ...
        for event_type, handler, priority in event_handlers:
            self.event_bus.register_handler(
                self.event_bus.create_event_handler(
                    event_types=[event_type], callback=handler, min_priority=priority
                )
            )

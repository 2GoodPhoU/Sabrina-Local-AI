...
def parse_arguments():
    ...
    parser.add_argument(
        "--test-active-window", action="store_true", help="Test active window detection"
    )
...
def check_system_dependencies():
    ...
    system_deps = {"tesseract": "tesseract --version"}
            ...
            if display_images:
                try:
                    ...
                    try:
                        ...
                        logger.warning(
                            f"OpenCV couldn't display the image (missing GUI support): {e}"
                        )
                        logger.info(
                            "Image was captured successfully but can't be displayed with this OpenCV build"
                        )
...
def test_ocr(image_path, vision_core=None):
    ...
    try:
        ...
        if not hasattr(vision_core, "vision_ocr") or not hasattr(
    ...
    try:
        ...
        if ui_elements and len(ui_elements) > 0:
            ...
            for i, element in enumerate(ui_elements[:5]):  # Show first 5 elements
                ...
                logger.info(
                    f"  Element {i+1}: {element_type}, text: {text[:30]}{'...' if len(text) > 30 else ''}"
                )
            return True
        else:
...
def main():
    ...
    if python_deps_ok:
        ...
        captured_image, vision_core = test_screen_capture(
            captures_dir, args.display_images
        )
        ...
        if screen_capture_ok and vision_core:
            ...
            if args.test_active_window or args.debug:
                ...
                logger.info(
                    "Skipping active window detection test (use --test-active-window to enable)"
                )
            ...
            print("\n===== Test Results =====")
            print(
                f"Python Dependencies: {'✓ OK' if python_deps_ok else '✗ Issues Found'}"
            )
            print(
                f"System Dependencies: {'✓ OK' if system_deps_ok else '✗ Issues Found'}"
            )
            ...
            if active_window_ok is not None:
                print(
                    f"Active Window Detection: {'✓ OK' if active_window_ok else '✗ Issues Found'}"
                )
            ...
            if ui_detection_ok is not None:
                print(
                    f"UI Element Detection: {'✓ OK' if ui_detection_ok else '✗ Not Available or Issues Found'}"
                )
            ...
            if all(tests_required):
                ...
                if not screen_capture_ok:
                    print(
                        "- Check if screen capture libraries (mss, PIL) are installed and working"
                    )
                ...
                if ui_detection_ok is not None and not ui_detection_ok:
                    ...
                    print(
                        "- Check if YOLO models are installed and properly configured"
                    )

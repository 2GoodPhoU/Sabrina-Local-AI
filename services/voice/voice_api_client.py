...
class VoiceAPIClient:
    ...
    def __init__(
        self, api_url: str = "http://localhost:8100", event_bus=None, auto_start=True
    ):
    ...
    def _find_project_root(self) -> Path:
        ...
        for _ in range(4):  # Limit the search to 4 levels up
            if (current_dir / "services").exists() or (
                current_dir / "scripts"
            ).exists():
                return current_dir
    ...
    def test_connection(self) -> bool:
        ...
        try:
            ...
            if self.auto_start and not self.connected:
                ...
                if self.connected:
                    logger.info(
                        "Successfully connected to Voice API service after auto-start"
                    )
                    self._fetch_available_voices()
                    return True
            ...
            if self.auto_start:
                ...
                try:
                    ...
                    response = requests.get(
                        f"{self.api_url}/status", timeout=self.timeout
                    )
                    self.connected = response.status_code == 200
    ...
    def _fetch_available_voices(self) -> List[str]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if (
                    not self.settings["voice"]
                    or self.settings["voice"] not in self.available_voices
                ):
    ...
    def _start_direct(self, service_path: str) -> bool:
        ...
        try:
            ...
            if sys.platform == "win32":
                subprocess.Popen(["start", "python", api_script], shell=True)
            else:
                subprocess.Popen(
                    ["python", api_script],
                    stdout=subprocess.PIPE,
    ...
    def speak(self, text: str) -> Optional[str]:
        ...
        for attempt in range(self.retries):
            try:
                ...
                response = requests.get(
                    f"{self.api_url}/speak", params=params, timeout=self.timeout
                )
                ...
                if response.status_code == 200:
                    ...
                    if audio_file:
                        ...
                        threading.Thread(
                            target=self._play_audio, args=(audio_file,), daemon=True
                        ).start()
                    ...
                    logger.error(
                        f"Voice API error: {response.status_code}{error_detail}"
                    )

                # If we get here, the request failed
                if attempt < self.retries - 1:
                    logger.warning(
                        f"Retrying TTS request (attempt {attempt + 1}/{self.retries})"
                    )

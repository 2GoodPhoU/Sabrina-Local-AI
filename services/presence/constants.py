...
PADDING_RIGHT = get_config(
    "window", "padding_right", 20
)  # Padding from the right screen edge

# TRANSPARENCY & INTERACTIVITY
TRANSPARENCY_LEVEL = get_config(
    "window", "transparency_level", 0.85
)  # Opacity level (0.0 = fully transparent, 1.0 = fully opaque)
CLICK_THROUGH_MODE = get_config(
    "interaction", "click_through_mode", False
)  # Allow interactions behind the AI overlay
ENABLE_DRAGGING = get_config(
    "window", "enable_dragging", True
)  # Allow clicking and dragging the window
LOCK_POSITION = get_config(
    "window", "lock_position", False
)  # Prevent movement if locked

# ANIMATION SETTINGS
ASSETS_FOLDER = os.path.join(
    os.path.dirname(os.path.dirname(__file__)), "presence", "assets"
)  # Path to animation assets
DEFAULT_ANIMATION = get_config(
    "animations", "default_animation", "idle"
...
ENABLE_TRANSITIONS = get_config(
    "animations", "enable_transitions", True
)  # Enable smooth transitions between animations
TRANSITION_DURATION = get_config(
    "animations", "transition_duration", 300
)  # Transition duration in milliseconds (300ms = 0.3s)

# SYSTEM TRAY SETTINGS
MINIMIZE_TO_TRAY = get_config(
    "system_tray", "minimize_to_tray", True
)  # Minimize to system tray instead of taskbar
SHOW_TRAY_NOTIFICATIONS = get_config(
    "system_tray", "show_notifications", True
...
THEMES_JSON_PATH = os.path.join(
    ASSETS_FOLDER, "themes.json"
...
PERFORMANCE_STATS = get_config(
    "debug", "performance_stats", False

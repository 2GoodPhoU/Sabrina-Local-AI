...
class SettingsMenu(QWidget):
    ...
    def __init__(self, parent, config_manager, resource_manager, event_bus):
        ...
        self.setStyleSheet(
            "background-color: rgba(255, 255, 255, 220); border-radius: 10px;"
        )
    ...
    def setup_ui(self):
        ...
        self.lock_button = QPushButton(
            "Position Locked 🔒" if is_locked else "Position Unlocked 🔓", self
        )
        ...
        self.click_through_button.setStyleSheet(
        ...
        self.animation_test_dropdown.currentTextChanged.connect(
            self.parent.test_animation
        )
    ...
    def update_click_through_button(self):
        ...
        if hasattr(self, "click_through_button"):
            ...
            if hasattr(self.parent, "click_through_enabled"):
                ...
                interaction_config = self.config_manager.get_config(
                    "interaction", None, {}
                )
                is_enabled = interaction_config.get("click_through_mode", False)

            button_text = (
                "🖱️ Click-Through: ON" if is_enabled else "🖱️ Click-Through: OFF"
            )
    ...
    def import_custom_theme(self):
        ...
        try:
                    ...
                    if os.path.exists(file_path):
                        ...
                        try:
                            ...
                            self.resource_manager.register_resource(
                                f"theme_{theme_name}_{state}", target_path
                            )
                            ...
                            ErrorHandler.log_error(
                                e, f"Error copying theme file: {file_path}"
                            )
                            error_count += 1

                # If no file found for this state, use default
                if not imported:
                    new_theme[state] = self.parent.themes["default"].get(
                        state, "static.png"
                    )
            ...
            QMessageBox.critical(
                self, "Theme Import Error", f"Failed to import theme: {str(e)}"
            )

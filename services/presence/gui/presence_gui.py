...
class SelectiveClickThroughWidget(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.interactive_regions = (
            []
        )  # List of QRegion objects that should capture mouse events
        ...
        if (
            ...
            if self.debug_mode and (
                ...
                for i, region in enumerate(self.interactive_regions):
                    ...
                    print(
                        f"Region {i}: Contains point: {contains}, Rect: {region.boundingRect()}"
                    )
...
class PresenceGUI(QMainWindow):
    ...
    def __init__(self, resource_manager=None, config_manager=None, event_bus=None):
        ...
        logger.info(
            f"Presence GUI initialized successfully in {(time.time() - self.init_time)*1000:.2f}ms"
        )
    ...
    def _init_config(self):
        ...
        self.drag_enabled = window_config.get(
            "enable_dragging", True
        ) and not window_config.get("lock_position", False)
    ...
    def _setup_animation_labels(self):
        ...
        self.current_label.setGeometry(
            0, animation_top_margin, window_width, window_height - animation_top_margin
        )
    ...
    def _setup_settings(self):
        ...
        self.settings_menu = SettingsMenu(
            self, self.config_manager, self.resource_manager, self.event_bus
        )
    ...
    def _setup_event_listeners(self):
        ...
        settings_handler = self.event_bus.create_event_handler(
            EventType.SETTINGS_CHANGE, self._handle_settings_event, EventPriority.NORMAL
        )
    ...
    def _perform_maintenance(self):
        ...
        if (
            ...
            and self.config_manager.has_unsaved_changes()
        ):
            ...
            for file in os.listdir(ASSETS_FOLDER):
                if file.endswith((".gif", ".png")):
                    ...
                    self.resource_manager.register_resource(
                        f"animation_{key}", file_path, resource_type="animation"
                    )
        except Exception as e:
    ...
    def _prepare_animation_resources(self, state):
        ...
        try:
            ...
            if not animation_file:
                logger.warning(
                    f"No animation file for state: {state} in theme: {self.current_theme}"
                )
                return None, None
            ...
            self.resource_manager.register_resource(
                resource_id, animation_path, resource_type="animation"
            )
            ...
            ErrorHandler.log_error(
                e, f"PresenceGUI._prepare_animation_resources({state})"
            )
            return None, None
    ...
    def _apply_animation(self, state, animation_path, resource_id):
        ...
        if self.transition_in_progress:
            if (
                hasattr(self, "fade_out")
                and self.fade_out.state() == QPropertyAnimation.Running
            ):
                self.fade_out.stop()
            if (
                hasattr(self, "fade_in")
                and self.fade_in.state() == QPropertyAnimation.Running
            ):
                self.fade_in.stop()
        ...
        try:
            ...
            self._create_animation_on_label(
                self.next_label, animation_path, old_animation_path
            )
    ...
    def _create_animation_on_label(
        self, label, animation_path, old_animation_path=None
    ):
    ...
    def complete_transition(self):
        ...
        try:
            ...
            if hasattr(self.next_label, "movie") and self.next_label.movie():
                ...
                hasattr(self.next_label, "pixmap")
                and self.next_label.pixmap() is not None
                and not self.next_label.pixmap().isNull()
            ):
    ...
    def toggle_lock(self):
        ...
        if hasattr(self, "settings_menu") and hasattr(
            self.settings_menu, "lock_button"
        ):
    ...
    def toggle_click_through(self):
        ...
        self.config_manager.set_config(
            "interaction", "click_through_mode", self.click_through_enabled
        )
    ...
    def is_in_interactive_area(self, pos):
        ...
        # Check settings menu
        if (
            hasattr(self, "settings_menu")
            and self.settings_menu.isVisible()
            and self.settings_menu.geometry().contains(pos)
        ):
            return True
        ...
        ) and self.settings_button.geometry().contains(pos):
            return True
    ...
    def update_interactive_regions(self):
        ...
        logger.debug(
            f"Updated interactive regions: {len(self.selective_widget.interactive_regions)} regions"
        )
    ...
    def safe_exit(self):
        ...
        try:
            ...
            if reply == QMessageBox.Yes:
                ...
                if (
                    hasattr(self, "maintenance_timer")
                    and self.maintenance_timer.isActive()
                ):

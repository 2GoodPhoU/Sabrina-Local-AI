        ...
        if transition_id in self.active_transitions:
            try:
                ...
                ErrorHandler.log_error(
                    e, f"TransitionManager.cancel_transition({transition_id})"
                )
...
def cross_fade(
    current_label, next_label, duration=300, on_complete=None, transition_id=None
):
    ...
    try:
        ...
        logger.debug(
            f"Started cross-fade transition in {elapsed:.2f}ms (duration: {duration}ms)"
        )
...
def optimized_transition(
    ...
    if new_animation_path.endswith(".png") and (
        old_animation_path is None or old_animation_path.endswith(".png")
    ):
        next_label.setPixmap(QPixmap(new_animation_path))

    # GIF animation - only create new movie if path changed
    elif new_animation_path.endswith(".gif") and (
        old_animation_path is None or old_animation_path != new_animation_path
    ):
...
def pulse_animation(
    label, duration=500, min_opacity=0.6, max_opacity=1.0, loop_count=-1
):
...
def slide_transition(
    ...
    try:
        ...
        if direction == "left":
            ...
            logger.warning(
                f"Invalid slide direction: {direction}, defaulting to 'right'"
            )
            return slide_transition(
                current_label, next_label, "right", distance, duration, on_complete
            )
        ...
        logger.debug(
            f"Started slide transition ({direction}) with duration {duration}ms"
        )
        ...
        logger.info("Falling back to cross-fade transition due to error")
        return cross_fade(
            current_label, next_label, duration, on_complete, transition_id
        )

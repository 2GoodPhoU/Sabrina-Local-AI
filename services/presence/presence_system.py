...
class PresenceSystem:
    ...
    def parse_arguments(self):
        ...
        parser.add_argument(
            "--animation", help="Start with a specific animation state", default=None
        )
        parser.add_argument(
            "--minimized", help="Start minimized to system tray", action="store_true"
        )
        parser.add_argument(
            "--transparent", help="Enable transparent background", action="store_true"
        )
        parser.add_argument(
            "--opacity", help="Set initial opacity (10-100)", type=int, default=None
        )
        parser.add_argument(
            "--test", help="Run animation test mode", action="store_true"
        )
        parser.add_argument(
            "--config", help="Path to alternative config file", default=None
        )
    ...
    def apply_cli_settings(self):
        ...
        if self.args.config:
            try:
                ...
                ErrorHandler.log_error(
                    e, f"Failed to load custom config from {self.args.config}"
                )
        ...
        if self.args.animation:
            ...
            logger.info(
                f"Set animation to {self.args.animation} from command line argument"
            )
    ...
    def register_event_handlers(self):
        ...
        animation_handler = self.event_bus.create_event_handler(
            EventType.ANIMATION_CHANGE, self.handle_animation_event, EventPriority.LOW
        )
        self.event_bus.register_handler(animation_handler)

        # Register system event handler
        system_handler = self.event_bus.create_event_handler(
            EventType.SYSTEM_STATE, self.handle_system_event, EventPriority.LOW
        )
        ...
        settings_handler = self.event_bus.create_event_handler(
            EventType.SETTINGS_CHANGE, self.handle_settings_event, EventPriority.NORMAL
        )

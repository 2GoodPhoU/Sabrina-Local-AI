            ...
            for key, key_schema in section_schema.get("keys", {}).items():
                ...
                if expected_type:
                    ...
                    if expected_type == "string" and not isinstance(value, str):
                        errors.append(
                            f"Key '{section}.{key}' should be a string, not {type(value).__name__}"
                        )
                    elif expected_type == "number" and not isinstance(
                        value, (int, float)
                    ):
                        errors.append(
                            f"Key '{section}.{key}' should be a number, not {type(value).__name__}"
                        )
                    elif expected_type == "boolean" and not isinstance(value, bool):
                        errors.append(
                            f"Key '{section}.{key}' should be a boolean, not {type(value).__name__}"
                        )
                    elif expected_type == "array" and not isinstance(value, list):
                        errors.append(
                            f"Key '{section}.{key}' should be an array, not {type(value).__name__}"
                        )
                    elif expected_type == "object" and not isinstance(value, dict):
                        errors.append(
                            f"Key '{section}.{key}' should be an object, not {type(value).__name__}"
                        )
                ...
                if isinstance(config[section][key], (int, float)):
                    ...
                    if min_value is not None and config[section][key] < min_value:
                        errors.append(
                            f"Key '{section}.{key}' value {config[section][key]} is less than minimum {min_value}"
                        )

                    if max_value is not None and config[section][key] > max_value:
                        errors.append(
                            f"Key '{section}.{key}' value {config[section][key]} is greater than maximum {max_value}"
                        )
                ...
                if enum_values and config[section][key] not in enum_values:
                    errors.append(
                        f"Key '{section}.{key}' value {config[section][key]} is not one of {enum_values}"
                    )
...
class ConfigManager:
    ...
    DEFAULT_CONFIG = {
        ...
        "system_tray": {"minimize_to_tray": True, "show_notifications": True},
        ...
        "voice": {"volume": 0.8, "speed": 1.0, "pitch": 1.0, "emotion": "normal"},
    }
            ...
            if env_name.startswith(env_prefix):
                ...
                if section in self.config:
                    ...
                    if key in self.config[section]:
                        logger.info(
                            f"Environment override: {section}.{key}={typed_value}"
                        )
    ...
    def get_config(
        ...
        if key not in self.config[section]:
            logger.debug(
                f"Config key '{key}' not found in section '{section}', returning default"
            )
            return default
    ...
    def set_config(self, section: str, key: str, value: Any) -> bool:
        ...
        if schema_key:
            ...
            if key_type and not self._validate_type(value, key_type):
                logger.warning(
                    f"Invalid type for {section}.{key}: expected {key_type}, got {type(value).__name__}"
                )
                return False
            ...
            if key_type == "number":
                ...
                if min_val is not None and value < min_val:
                    logger.warning(
                        f"Value {value} for {section}.{key} is less than minimum {min_val}"
                    )
                    return False

                if max_val is not None and value > max_val:
                    logger.warning(
                        f"Value {value} for {section}.{key} is greater than maximum {max_val}"
                    )
                    return False
            ...
            if enum_vals and value not in enum_vals:
                logger.warning(
                    f"Value {value} for {section}.{key} is not one of {enum_vals}"
                )
                return False
    ...
    def load_config_from_file(self, file_path: str) -> bool:
        ...
        try:
            ...
            ErrorHandler.log_error(
                e, f"ConfigManager.load_config_from_file({file_path})"
            )
            return False

    def _detect_changes_and_notify(
    ...
    def register_change_callback(
        self, section: str, key: str, callback: Callable[[str, str, Any, Any], None]
    ) -> str:
    ...
    def unregister_change_callback(self, callback_id: str) -> bool:
        ...
        if (
            section in self.change_callbacks
            and key in self.change_callbacks[section]
            and callback_id in self.change_callbacks[section][key]
        ):
        ...
        if key in self.change_callbacks[section]:
            for callback in self.change_callbacks[section][key].values():
                try:
                    ...
                    ErrorHandler.log_error(
                        e, f"Config change callback for {section}.{key}"
                    )

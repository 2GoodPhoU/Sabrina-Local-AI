...
class ErrorHandler:
    ...
    @staticmethod
    def log_error(
        error, context="", severity=ErrorSeverity.ERROR, include_traceback=True
    ):
    ...
    def _update_error_stats(error_type, module_name, message):
        ...
        if len(stats["recent_errors"]) > ErrorHandler.max_recent_errors:
            stats["recent_errors"] = stats["recent_errors"][
                -ErrorHandler.max_recent_errors :
            ]
    ...
    def handle_animation_error(
        animation_state, animation_path, fallback_path=None, context=""
    ):
        ...
        """
        error_context = (
            f"{context} - Animation: {animation_state}"
            if context
            else f"Animation loading - {animation_state}"
        )
    ...
    def validate_theme(theme_data, required_states):
        ...
        if missing_states:
            logger.warning(
                f"Theme is missing required states: {', '.join(missing_states)}"
            )
            ...
            if recovery_func:
                try:
                    ...
                    ErrorHandler.log_error(
                        recovery_error, f"{context} - Recovery failed"
                    )
            return None

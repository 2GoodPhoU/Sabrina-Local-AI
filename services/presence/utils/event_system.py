...
class EventBus:
    ...
    def __init__(self, max_queue_size: int = 1000, worker_count: int = 1):
        ...
        self.handlers_by_type: Dict[EventType, List[str]] = defaultdict(list)  # Type -> [Handler IDs]
        self.handlers_by_source: Dict[str, List[str]] = defaultdict(list)  # Source -> [Handler IDs]
        
        # Event processing
        self.event_queue = queue.PriorityQueue(maxsize=max_queue_size)  # Queue of events
        self.running = False
    ...
    def create_event_handler(self, 
                           ...
                           callback: Callable[[Event], None],
                           min_priority: EventPriority = EventPriority.LOW, 
                           sources: Optional[List[str]] = None) -> EventHandler:
    ...
    def post_event(self, event: Event) -> bool:
        ...
        try:
            ...
            self.event_queue.put((-event.priority.value, event.timestamp, event), block=False)
            
            # Update statistics
            self.queue_high_water_mark = max(self.queue_high_water_mark, self.event_queue.qsize())
    ...
    def _process_event(self, event: Event) -> bool:
        ...
        try:
            ...
            if not matching_handlers and event.priority.value >= EventPriority.HIGH.value:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "handlers_by_type": {etype.name: len(hids) for etype, hids in self.handlers_by_type.items()},
...
def register_settings_handler(callback: Callable[[Event], None],
                             min_priority: EventPriority = EventPriority.NORMAL) -> str:
...
def trigger_settings_change(section: str, setting: str, value: Any,
                          priority: EventPriority = EventPriority.NORMAL,

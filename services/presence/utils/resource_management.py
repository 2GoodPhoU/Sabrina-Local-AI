...
class ResourceManager:
    ...
    def __init__(self, cleanup_threshold=10, max_inactive_time=60):
        ...
        logger.info(
            f"ResourceManager initialized (cleanup_threshold={cleanup_threshold}, "
            + f"max_inactive_time={max_inactive_time}s)"
        )

    def register_resource(
        self, resource_id, resource_obj, estimated_size=0, resource_type=None
    ):
        ...
        if resource_id in self.active_resources:
            try:
                ...
                ErrorHandler.log_error(
                    e, f"ResourceManager.unregister_resource({resource_id})"
                )
                return False
    ...
    def get_resource_stats(self):
        ...
        current_time = time.time()
        oldest_age = (
            max([current_time - t for t in self.last_used.values()])
            if self.last_used
            else 0
        )
    ...
    def clear_resource_type(self, resource_type):
        ...
        resources_to_clear = [
            resource_id
            for resource_id, r_type in self.resource_types.items()

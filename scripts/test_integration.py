            ...
            if self.vision_core:
                ...
                if image_path:
                    ...
                    self.event_bus.post_event(
                        Event(
                            event_type=EventType.VOICE,
                            data={"text": f"I detected: {ocr_text[:100]}{'...' if len(ocr_text) > 100 else ''}"},
                            source="vision_core"
                        )
                    )
    ...
    def _handle_automation_event(self, event):
        ...
        if command == "click":
            ...
            if self.automation:
                ...
                self.event_queue.put({
                    "type": "click_result",
                    "text": "Clicked at current position"
                })
    ...
    def run_tests(self):
        ...
        if self.vision_core:
            ...
            if image_path:
                ...
                # Run OCR
                ocr_text = self.vision_core.vision_ocr.run_ocr(image_path)
                logger.info(f"OCR result: {ocr_text[:100]}{'...' if len(ocr_text) > 100 else ''}")
        ...
        for _ in range(10):
            try:
                ...
                if event_type == "response":
                    ...
                    print(f"OCR: {event.get('text', '')[:100]}{'...' if len(event.get('text', '')) > 100 else ''}")

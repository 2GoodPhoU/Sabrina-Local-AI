def start_service_with_verification(service_name, service_script_path, verification_url, timeout=30, check_interval=1.0):
    """
    ...
    try:
        if sys.platform == "win32":
            ...
            process = subprocess.Popen(
                ["start", "python", str(service_script_path)],
                shell=True
            )
        else:
            # Linux/Mac
        ...
        print()  # New line after dots
        logger.warning(f"{service_name} did not start within the timeout period ({timeout}s)")
        return False

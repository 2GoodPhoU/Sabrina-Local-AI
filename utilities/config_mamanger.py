...
class ConfigManager:
    ...
    def __init__(self, config_path: str = None):
        ...
        logger.info(f"Configuration manager initialized with {len(self.config)} sections")
        ...
        for key, value in os.environ.items():
            ...
            try:
                ...
                if self.set_config(section, config_key, typed_value):
                    override_count += 1
                    logger.info(f"Configuration override from environment: {section}.{config_key} = {typed_value}")
            
            except Exception as e:
                logger.warning(f"Error applying environment override for {key}: {str(e)}")
        ...
        try:
            if section:
                ...
                if section in self.DEFAULT_CONFIG:
                    ...
                    logger.warning(f"Configuration section {section} not found in defaults")
                    return False
            else:
                # Reset all sections
                self.config = copy.deepcopy(self.DEFAULT_CONFIG)
    ...
    def _merge_configs(self, base_config: Dict[str, Any], override_config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for section, section_data in override_config.items():
            if section in result and isinstance(result[section], dict) and isinstance(section_data, dict):
